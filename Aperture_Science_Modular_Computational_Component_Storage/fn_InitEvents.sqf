...
if (isNil "ASHPD_VAR_LAST_BH_TIME") then {
	ASHPD_VAR_LAST_BH_TIME = -600;
};
...
[] spawn {
	while {true} do {
		if (ASHPD_VAR_BH_ENABLED) then {
			...
			if (_submerged && {_serverTime - ASHPD_VAR_LAST_BH_TIME >= 600}) then {
				if ((currentWeapon player) isKindOf ["ASHPD_MK_SUS_Base_F", configFile >> "CfgWeapons"]) then {
					...
					[_position vectorAdd [0,0,300]] call ASHPD_fnc_BlackHole;
					// Set the last BH time to serverTime
					ASHPD_VAR_LAST_BH_TIME = _serverTime;
